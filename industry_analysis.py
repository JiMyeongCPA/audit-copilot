...
# 각 재무비율이 '어떤 계정들로 계산되는지'를 공식에서 그대로 뽑아둔 표.
# compute_company_ratios 위쪽 공식과 1:1로 대응함 (공식을 바꾸면 여기도 같이 바꿔야 함).
# AI에 넘길 '관련 계정'을 사람이 임의로 고르지 않고 이 공식 구성에서 기계적으로 유도하기 위한 것.
RATIO_COMPONENTS = {
    "매출채권회전율": ["매출액", "매출채권"],
    "매출채권평균회수기간": ["매출채권", "매출액"],
    "매출채권매출액비율": ["매출채권", "매출액"],
    ...
    "장기차입금자산비율": ["장기차입금", "자산총계"],
    "금융원가차입금비율": ["금융원가", "단기차입금", "유동성장기부채", "장기차입금"],
    "ROE": ["당기순이익", "자본총계"],
    "ROA": ["당기순이익", "자산총계"],
    "총자산회전율": ["매출액", "자산총계"],
    "매출원가율": ["매출원가", "매출액"],
    "유형자산회전율": ["매출액", "유형자산"],
    "유형자산총자산비율": ["유형자산", "자산총계"],
    "사용권자산총자산비율": ["사용권자산", "자산총계"],
    "무형자산총자산비율": ["무형자산", "자산총계"],
    "영업권총자산비율": ["영업권", "자산총계"],
    "투자부동산총자산비율": ["투자부동산", "자산총계"],
    "관계기업투자총자산비율": ["관계기업투자", "자산총계"],
    "이연법인세자산자본비율": ["이연법인세자산", "자본총계"],
    "충당부채매출액비율": ["유동충당부채", "비유동충당부채", "매출액"],
    "계약부채매출액비율": ["계약부채", "매출액"],
    "자기주식자본비율": ["자기주식", "자본총계"],
    "매출총이익률": ["매출총이익", "매출액"],
    "판관비율": ["판매비와관리비", "매출액"],
    "금융수익매출액비율": ["금융수익", "매출액"],
    "세전이익률": ["법인세비용차감전순이익", "매출액"],
    "유효세율": ["법인세비용", "법인세비용차감전순이익"],
    "영업이익률": ["영업이익", "매출액"],
    "이자보상배율": ["영업이익", "금융원가"],
    "순이익률": ["당기순이익", "매출액"],
    "OCF매출액비율": ["영업활동현금흐름", "매출액"],
    "투자현금흐름부담률": ["투자활동현금흐름", "매출액"],
    "외부자금의존도": ["재무활동현금흐름", "영업활동현금흐름"],
    "CAPEX커버리지": ["영업활동현금흐름", "유형자산취득액", "무형자산취득액"],
    "자산처분현금비율": ["유형자산처분액", "무형자산처분액", "매출액"],
    "영업현금배당지급률": ["배당금지급액", "영업활동현금흐름"],
    "현금세율": ["법인세납부액", "법인세비용차감전순이익"],
    "현금이자율": ["이자지급액", "단기차입금", "유동성장기부채", "장기차입금"],
    "신규차입률": ["단기차입금유입액", "장기차입금유입액", "단기차입금", "유동성장기부채", "장기차입금"],
    "차입금상환률": ["단기차입금상환액", "장기차입금상환액", "단기차입금", "유동성장기부채", "장기차입금"],
    "리스부채상환률": ["리스부채상환액", "유동리스부채", "비유동리스부채"],
}
